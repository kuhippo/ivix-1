...
true_ivix = {}
try:
    with open('/Users/mubin/Desktop/python/ivix-master/shibor.csv', 'r', encoding='gbk') as f:
        shibor_rate = pd.read_csv(f, index_col=0, encoding='GBK')
except UnicodeDecodeError as e:
    print(e)

try:
    with open('/Users/mubin/Desktop/python/ivix-master/options.csv', 'r', encoding='gbk') as f:
        options_data = pd.read_csv(f, index_col=0, encoding='GBK')
except UnicodeDecodeError as e:
    print(e)

try:
    with open('/Users/mubin/Desktop/python/ivix-master/tradeday.csv', 'r', encoding='gbk') as f:
        tradeday = pd.read_csv(f, index_col=0, encoding='GBK')
except UnicodeDecodeError as e:
    print(e)

try:
    with open('/Users/mubin/Desktop/python/ivix-master/ivixx.csv', 'r', encoding='gbk') as f:
...
if __name__ == '__main__':
    ...
    Line().add_xaxis(attr).add_yaxis("中证指数发布", true_ivix[u'收盘价(元)'].tolist(), is_smooth=True,markpoint_opts=['average','max']).add_yaxis("公式计算数据", ivix, is_smooth=True,markline_opts=['max']).render("/Users/mubin/Desktop/python/ivix-master/vix.html")
